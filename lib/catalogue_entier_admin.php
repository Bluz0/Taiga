...
<body>
    ...
    <header>
        ...
        <div class="searchbox">
            <input type="text" id="search" onkeyup="search()" placeholder="Rechercher un article">
        </div>

        <div class="group">
            <ul class="navigation">
            <li><a href="admin.php">Accueil</a></li>
                <li><a href="admin_usr.php">Gestion des utilisateurs</a></li>
                <li><a href="catalogue_entier_admin.php">Gestion des articles</a></li>
    ...
    <div id="ajouter_prd"></div>
  </div>
    <div id=catalogue_ajout>
...
//fin du script pour  la barre du filtre de prix
        fetch("article_json.php").then(json_to_data).then(onglet_article);
        fetch("article_json.php").then(json_to_data).then(produit_card);
        ...
        function ajout_article(){
            div_ajout=document.getElementById("ajouter_prd");
            lien=document.createElement("a");
            lien.href="admin_ajout_article.php";
            lien.innerHTML="Ajouter un article";
            div_ajout.appendChild(lien);
        }
        ...
        function produit_card(data){ //Utilise les donnees pour construire l'interface
            ...
            for(let i = 0; i < data.length; i++){ 
                ...
                div.setAttribute("onclick","location.href='admin_article.php?id="+lien+"'"); //Accède à la page de l'article
		...
		function filtres_cat_prix() {
			filtres_cat();
			setTimeout(() => { filtre_prix();}, 500);
		}
